...
@section('main.content')
    <form action="{{ route('complaint.store') }}" method="post" class="needs-validation simple-form-long">
        @CSRF
        <legend>{{ __('main.complaint_create_view') }}</legend>
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <select class="form-select form-select @error('status') validation-issue text-red @enderror" name="status" aria-label=".form-select-sm example">
                    ...
                    <option value="Registered" @if (old('status') == 'Registered') selected @endif>Registered</option>
                    <option value="Solving" @if (old('status') == 'Solving') selected @endif>Solving</option>
                    <option value="Resolved" @if (old('status') == 'Resolved') selected @endif>Resolved</option>
                    <option value="Spam" @if (old('status') == 'Spam') selected @endif>Spam</option>
                    <option value="Inform client" @if (old('status') == 'Inform client') selected @endif>Inform client</option>
                </select>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <select class="form-select form-select @error('category') validation-issue text-red @enderror" name="category" aria-label=".form-select-sm example">
                    ...
                    <option value="System not working" @if (old('category') == 'System not working') selected @endif>System not working</option>
                    <option value="Need permissions" @if (old('category') == 'Need permissions') selected @endif>Need permissions</option>
                    <option value="Found bug" @if (old('category') == 'Found bug') selected @endif>Found bug</option>
                    <option value="API" @if (old('category') == 'API') selected @endif>API</option>
            ...
            <div class="col-md-4 mb-3">
                ...
                <select class="form-select form-select @error('system_id') validation-issue text-red @enderror" name="system_id" aria-label=".form-select-sm example">
                    @error('system_id') <option selected>{{ $message }}</option> @enderror
                    @if(old('system_id'))
                        @foreach($systems as $system)
                            @if(old('system_id') == $system->id)
