    ...
    <form action="{{ route('complaint.update', ($complaint->id ?? '')) }}" method="post" class="needs-validation simple-form-long">
        ...
        <div class="row">
            <div class="col-md-4 mb-3">
                ...
                <input type="text" name="name" class="form-control @error('name') validation-issue text-red @enderror" @error('name') value="{{ $message }}" @else value="{{ $complaint->name }}" @enderror placeholder="Name" required>
            </div>
            <div class="col-md-4 mb-3">
                ...
                <select class="form-select form-select @error('assigned_user_id') validation-issue text-red @enderror" name="assigned_user_id" aria-label=".form-select-sm example">
                    ...
                    @if($complaint->user->name)
                        <option value="{{ $complaint->user->id }}" selected>{{ $complaint->user->name }}</option>
                    @endif
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <select class="form-select form-select @error('system_id') validation-issue text-red @enderror" name="system_id" aria-label=".form-select-sm example">
                    ...
                    @if($complaint->system->name)
                        <option value="{{ $complaint->system->id }}">{{ $complaint->system->name }}</option>
                    @endif
        ...
        <div class="row">
            <div class="col-mb-12 mb-3">
                ...
                <textarea class="form-control @error('description') validation-issue text-red @enderror" name="description" rows="3">@error('description') {{ $message }} @else {{ $complaint->description }} @enderror</textarea>
            </div>
        </div>
        <div class="row">
