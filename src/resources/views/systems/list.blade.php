...
<div class="d-flex flex-column bd-highlight mb-3">
    <div class="d-flex flex-column justify-content-center flex-lg-wrap">
        <form action="{{ route('system.list') }}" method="get" class="needs-validation simple-form-long">
{{--            <input type="datetime-local" name="birthdaytime">--}}
            @CSRF
            <legend>{{ __('main.systems_list_header') }}</legend>
            <div class="row">
                <div class="col-md-6 mb-6">
                    <label>{{ __('main.name') }}:</label>
                    <input type="text" name="name" class="form-control" value="{{ $systemsFilter->name ?? '' }}">
                </div>
                <div class="col-md-6 mb-6">
                    <label>{{ __('main.status') }}:</label>
                    <select class="form-select form-select" name="status" aria-label=".form-select-sm example">
                        <option></option>
                        <option value="Active" @if (($systemsFilter->status ?? '') == 'Active') selected @endif>Active</option>
                        <option value="Inactive" @if (($systemsFilter->status ?? '') == 'Inactive') selected @endif>Inactive</option>
                    </select>
                </div>
{{--                {{ dd($systemsFilter->created_at) }}--}}
                <div class="col-md-6 mb-6">
                    <label>{{ __('main.created_at_from') }}:</label>
                    <input type="date" name="created_at" class="form-control" value="{{ $systemsFilter->created_at ?? '' }}">
                </div>
                <div class="col-md-6 mb-6">
                    <label>{{ __('main.updated_at_from') }}:</label>
                    <input type="date" name="updated_at" class="form-control" value="{{ $systemsFilter->updated_at ?? '' }}">
                </div>
                <div class="col-md-6 mb-6">
                ...
                <div class="col-md-6 mb-6">
                    ...
                    <select class="form-select form-select" name="deleted" aria-label=".form-select-sm example">
                        <option></option>
                        <option value="0" @if (($systemsFilter->status ?? '') == '0') selected @endif>False</option>
                        <option value="1" @if (($systemsFilter->status ?? '') == '1') selected @endif>True</option>
                    </select>
                </div>
            ...
            <table class="table table-striped table-hover">
                ...
                <tr class="list-header">
                    ...
                    <th scope="col"><button type="submit" class="btn btn-light" name="sort" value="updated_at">{{ __('main.updated_at') }}</button></th>
                    ...
                    <th scope="col"><button type="submit" class="btn btn-light" name="sort" value="deleted">{{ __('main.deleted') }}</button></th>
                </tr>
                </thead>
                <tbody>
                @foreach ($systems as $system)
                    <tr>
                        ...
                        <td>{{ $system->status }}</td>
                        <th>{{ $system->assigned_user_id }}</th>
                        <td>@if($system->deleted == '0') False @elseif($system->deleted == '1') True @endif </td>
                    </tr>
                @endforeach
                </tbody>
            </table>
...
<div class="d-flex justify-content-end bd-highlight mb-3">
    {{ $systems->withQueryString()->appends(Request::except('page'))->links('pagination::bootstrap-4') }}
</div>
