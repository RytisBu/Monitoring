    ...
    <table class="table table-striped">
        ...
        <tr>
            <th scope="col" colspan="5" rowspan="2">
                <legend>{{ __('main.system_detail_view') }}</legend>
            </th>
        </tr>
        </thead>
        <tbody>
        ...
        <tr>
            ...
            <td>
                @foreach($users as $user)
                    @if($user->id == ($system->assigned_user_id ?? ''))
                        {{ $user->name }}
                    @endif
                @endforeach
            </td>
        ...
        <tr>
            ...
            <td colspan="4">{{ $system->description ?? '' }}</td>
        </tr>
        <tr class="detailview-subtitles">
        ...
        <tr>
            ...
            <td>{{ __('dropdowns.deleted.' . ($system->deleted ?? '')) }}</td>
        </tr>
        </tbody>
    </table>
@endsection
...
@section('sidebar.collection')
        ...
        <li class="nav-item side-nav">
            ...
            @if (in_array(Route::currentRouteName(), ['system.edit'])) active @else text-white @endif
                ">
            <span>
                {{ __('actions.edit') }}
            </span>
